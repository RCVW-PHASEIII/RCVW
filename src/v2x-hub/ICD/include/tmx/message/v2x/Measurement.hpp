...
public:
	...
	common::types::Enum<UnitType> get_unit() const { return unit; }
...
#undef _OPER_OVERLOAD
	...
	friend std::ostream &operator<<(std::ostream &os, const self &m) {
		...
		os << m.get_value() << (::isalnum(m.get_unit().get_enum_name()[0]) ? " " : "" ) << m.get_unit().get_enum_name();
		return os;
	}
	...
	friend std::istream &operator>>(std::istream &is, self &m) {
		...
		if (!unitStr.empty()) {
			common::types::Enum<UnitType> inUnit(unitStr.c_str());
...
/*	static void table(std::ostream &os, size_t colWidth = 12) {
		auto _units = common::types::Enum<UnitType>::AllValues();
