...
TmxQpidProtonClient::TmxQpidProtonClient() noexcept {
    ...
    this->register_broker("sb");        // MS Azure Service Bus
}
...
TmxError to_error(const proton::error_condition &ec) noexcept {
...
proton::error_condition to_error(const common::TmxError &err) noexcept {
    proton::map<typename types::String_::value_type, proton::scalar> map;
