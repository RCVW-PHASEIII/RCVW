...
uint64_t Clock::GetMillisecondsSinceEpoch(struct timespec &ts) {
    std::chrono::seconds _sec { ts.tv_sec };
    std::chrono::nanoseconds _ns { ts.tv_nsec };
    return std::chrono::duration_cast<std::chrono::milliseconds>(_sec + _ns).count();
}
...
uint64_t Clock::GetMillisecondsSinceEpoch(std::string timestring, std::string format)
{
	tm time;
	time.tm_isdst = -1;

	::strptime(timestring.c_str(), format.c_str(), &time);

	uint64_t t = ::mktime(&time);

	// Adjust for UTC time
	long offset = 0L;
	tm *gmtime = ::gmtime(&offset);
