...
#include <tmx/plugin/utils/interxn/RoadwayList.hpp>
...
std::pair<Roadway*, bool> RoadwayList::LoadMap(MapData &msg) {
	int roadwayId = -1;
    if (msg.roadSegments && msg.roadSegments->list.count)
        roadwayId = msg.roadSegments->list.array[0]->id.id;
