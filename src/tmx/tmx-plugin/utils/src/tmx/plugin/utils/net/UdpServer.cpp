...
#include <tmx/plugin/utils/net/UdpServer.hpp>
...
UdpServer::UdpServer(const std::string &address, int port)
    ...
    if (r != 0 || _addrInfo == NULL) {
        throw UdpServerRuntimeError(
                ("invalid address or port for UDP socket: \"" + address + ":" + decimalPort + "\"").c_str());
    }
    ...
    if (_socket == -1) {
        ...
        throw UdpServerRuntimeError(
                ("could not create UDP socket for: \"" + address + ":" + decimalPort + "\"").c_str());
    }
    ...
    if (r != 0) {
        ...
        close(_socket);
        throw UdpServerRuntimeError(
                ("could not bind UDP socket with: \"" + address + ":" + decimalPort + "\"").c_str());
    }
}
