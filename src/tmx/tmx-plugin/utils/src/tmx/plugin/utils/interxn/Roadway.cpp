...
#include <tmx/plugin/utils/interxn/Roadway.hpp>
 ...
 */
RoadSegmentList *FindRoadSegments(MapData &msg);
 ...
 * Only loads if newer
 */
bool Roadway::LoadMap(MapData &msg) {
...
bool Roadway::IsPedestrianReportedForLane(int laneId, SPAT &msg) {
 ...
 */
MapLane Roadway::GetMapLane(MapData &msg, int laneId)
{
	MapLane mapLane;
 ...
 */
std::list<MapLane> Roadway::GetMapLanes(MapData &msg) {
 ...
 */
std::list<LaneNode> Roadway::GetLaneNodes(MapData &msg, int laneId, double totalXOffset, double totalYOffset) {
