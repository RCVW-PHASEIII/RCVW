...
{
    return msg.intersections;
}

RoadSegmentList *FindRoadSegments(MapData &msg)
{
    return msg.roadSegments;
}
	...
	for (int i = 0; i < laneCount; i++)
	{
		...
		if (lane->nodeList.present == DEFINED_NODE)
		{
			for (int j = 0; j < lane->nodeList.choice.nodes.list.count; j++)
			{
				...
				LaneNode laneNode;
				laneNode.Point = GetPointFromNode(refPoint, x, y, totalXOffset, totalYOffset, elevation,
			...
			for (int j = 0; j < laneCount; j++)
			{
				...
				for (int k = 0; k < referenceLane->nodeList.choice.nodes.list.count; k++)
				{
					Node *node = referenceLane->nodeList.choice.nodes.list.array[k];
						...
						laneNode.Point = GetPointFromNode(refPoint, x, y, totalComputedXOffset, totalComputedYOffset, elevation, totalElevationOffset);
						laneNodes.push_back(laneNode);
				}
			}
		}
