 ...
 */
IntersectionGeometryList *FindIntersections(MapData &msg);
IntersectionStateList *FindIntersections(SPAT &msg);
 ...
 * Only loads if newer
 */
bool Intersection::LoadMap(std::shared_ptr<MapData> msg) {
    if (!msg) return false;
...
//	bool Intersection::LoadMap(ParsedMap& parsedMap) { //MapDataMessage &msg) {
	...
	int thisRev = msg->msgIssueRevision;
...
bool Intersection::UpdateSpat(std::shared_ptr<SPAT> msg)
{
    if (!msg) return false;
...
bool Intersection::IsPedestrianReportedForConnectionId(int connectionId) {
    ...
    std::shared_ptr<SPAT> spatCopy;
    {
        lock_guard<mutex> lock(spatLock);
        spatCopy = SpatMsg;
    }

	auto *states = FindIntersections(*spatCopy);
	if (!states)
		return false;
	...
	for(int i=0; i < states->list.count; i++)
	{
		...
		for(int j = 0; j < intersectionState->maneuverAssistList->list.count; j++)
		{
			...
			if(connectionId == connId)
			{
				...
				TLOG(DEBUG) << "+++++ Pedestrian:" << isPed << " Id:" << connId;
				return isPed;
			}
...
bool Intersection::DoesSpatMatchMap(SPAT &msg) {
...
bool Intersection::IsSignalForGroupRedLight(SPAT &msg, int signalGroup) {
 ...
 */
MapLane Intersection::GetMapLane(MapData &msg, int laneId) {
	MapLane mapLane;
 ...
 */
std::list<MapLane> Intersection::GetMapLanes(MapData &msg) {
 ...
 */
std::list<LaneNode> Intersection::GetLaneNodes(MapData &msg, int laneId, double totalXOffset, double totalYOffset) {
...
double Intersection::TimeRemainingForSignalGroup(int signalGroupId)
{
	...
	auto interX = FindIntersections(*spatCopy);
...
double Intersection::GetCurrentGrade(double lat, double lon)
{
    ...
    std::shared_ptr<MapData> fullCopy;
    {
        lock_guard<mutex> lock(mapLock);
        fullCopy = MapMessage;
    }
    ...
    std::list<LaneNode> t_nodes = GetLaneNodes(*fullCopy, r.LaneNumber, 0.0, 0.0);
...
MapMatchResult Intersection::GetApproachLaneForLocation(double lat, double lon, double heading)
{
	...
	{
        MapLane lane = this->GetMapLane(*fullCopy, r.LaneNumber);
