    ...
    if (json_obj.AppId == 6 /* EPCW */) {
        ...
        switch (json_obj.EventCode) {
            case "3": // Entered Area
                if (screenNotification) {
                    clearImage();
                    populateHisImageMessageObject(6, json_obj.EventID, json_obj.Severity);
        ...
        switch (json_obj.EventCode) {
            case "1": // Entered Area
                if (screenNotification) {
                    clearImage();
                    populateHisImageMessageObject(7, json_obj.EventID, json_obj.Severity);
        ...
        switch (json_obj.EventCode) {
            ...
            case "36":
                if (json_obj.Severity == "0") {
                    populateHisImageMessageObject(14, json_obj.EventID, json_obj.Severity);
                    commonBackgroundImage = "../images/Common/Background.png";
                    clearImage();
                    displayNextAlert();
                    nextAudio = -1;
...
function handleAppMessage(json_obj)
{
    ...
    if (json_obj.AppId == 2) // FCW
    {
        ...
        case "1":
            ...
            {
                if (screenNotification) {
                    populateHisImageMessageObject(2, json_obj.EventID, json_obj.Severity);
                }
                FCWSeverity = 1;
                // TODO Change
                nextAudio = 4;
    ...
    else if (json_obj.AppId == 3) // EEBL
    {
        ...
        case "0":
            if (screenNotification) {
                setTimedImage('../images/FCW/3-base-art.png', json_obj.CustomText, json_obj.DisplayDuration);
                populateHisImageMessageObject(3, json_obj.EventID, json_obj.Severity);
            }
            break;
        case "1":
        case "2":
    ...
    else if (json_obj.AppId == 1) // CSW
    {
        ...
        case "1": // Entered Area
            if (!CSWActive)
            {
                ...
                if (screenNotification) {
                    clearImage();
                    populateHisImageMessageObject(1, json_obj.EventID, json_obj.Severity);
    ...
    else if (json_obj.AppId == 10) // RCVW
    {
        ...
        case "1": // Entered Area
            ...
            if (screenNotification) {
                clearImage();
                populateHisImageMessageObject(10, json_obj.EventID, json_obj.Severity);
...
function TspwDisplayAlert(json_obj) {
    switch (json_obj.EventCode) {
        case "3": // Entered Area
            TSPWActive = true;
            clearImage();
            populateHisImageMessageObject(json_obj.AppId, json_obj.EventID, json_obj.Severity);
            TspwDisplayBase(json_obj);
            TspwClearAllZones();
            console.log("Displayed TSPW");
            break;
        case "4": // Exited Area
            TSPWActive = false;
            populateHisImageMessageObject(json_obj.AppId, json_obj.EventID, json_obj.Severity);
            TspwClearAllZones();
            clearImage();
            console.log("Stopped displaying TSPW");
            displayNextAlert();
            break;
        case "5": // Detected Area
            // No Action
    ...
    if (sev in tspwValidSeverity) {
		...
		if (imgFile) {
			...
			if (json_obj.DisplayDuration == null || json_obj.DisplayDuration == undefined || isNaN(json_obj.DisplayDuration) || json_obj.DisplayDuration == "" || json_obj.DisplayDuration == 0) {
			    ...
			    zoneObj.timerId = setTimeout(TspwClearZone, json_obj.DisplayDuration, evCode);
			}
