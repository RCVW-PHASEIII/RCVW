...
var commonLibraryJsVersion = "3.3.0";
var commonBackgroundImage = "../images/RCVW/System Ready.png";
var activeBackgroundImage = "../images/RCVW/System Ready.png";
var VTRWSeverity = 0;
var CSWSeverity = 0;
var EEBLSeverity = 0;
var FCWSeverity = 0;
		...
		if (tempObj.url == url) 
		{
			if (!tempObj.connecting)
			{
				...
				tempObj.socket.onopen = function(e) {
					...
					var newMess =  "TEST;" + '{"header":{"type":"__register","encoding":"JSON","timestamp":' + n + ',"flags":0},"payload":{"name":"TMX UI ' + uiString + '","description":"UI Interface","version":"' + commonLibraryJsVersion + '","exeLocation": "/Debug/UIManafer","coreIpAddr": "127.0.0.1","corePort": 24601,"messageTypes": [],"configuration": []}}';
					sendCommand(newMess);
					// Subscribe
...
function populateHisConfigMessageObject(preState, postState) {
    ...
    hisConfig.payload.Id = generateUUID();
    hisConfig.payload.HISType = 3;
    hisConfig.payload.StateChangeTimestamp = new Date().getTime();
    hisConfig.payload.TriggeringAlertType = "0";
    hisConfig.payload.TriggeringAlertId = "";
    hisConfig.payload.Severity = "0";
...
//        console.log("setImage:" + JSON.stringify(his));
...
function setTimedImage(image, displayText, duration)
{
    ...
    if ($("#ActiveImage").attr('src') != image)
    {
        his.payload.HISPreState = $("#ActiveImage").attr('src');
        his.payload.HISPostState = image;
        console.log("setTimedImage:" + JSON.stringify(his));
        sendCommand("TEST;" + JSON.stringify(his));     
       $("#ActiveImage").attr('src', image);
    }
//    $("#bottomDiv").html(displayText);
    ...
    stopTimer();
    if (duration == null || duration == undefined || isNaN(duration) || duration == "" || duration == 0) {
...
//           console.log("playAudio:" + JSON.stringify(his));
