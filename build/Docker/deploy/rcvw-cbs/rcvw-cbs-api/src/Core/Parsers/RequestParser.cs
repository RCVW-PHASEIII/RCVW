...
namespace Azure.DataApiBuilder.Core.Parsers
{
    ...
    public class RequestParser
    {
        ...
        public static void ParseQueryString(RestRequestContext context, ISqlMetadataProvider sqlMetadataProvider,IQueryEngineFactory _queryEngineFactory)
        {
            if ( 0 == context.ParsedQueryString!.Keys.Count ) {
                throw new DataApiBuilderException("Permission denied",HttpStatusCode.Forbidden,DataApiBuilderException.SubStatusCodes.AuthorizationCheckFailed);
            }

            Boolean uuidEncountered = false;
            ...
            foreach (string key in context.ParsedQueryString!.Keys)
            {
                ...
                if ( string.Equals(key,"uuid" ) ) {

                    string uuid = $"{context.ParsedQueryString[key]}";
                    FindRequestContext frContext = new("Clients", context.DatabaseObject, false);
                    frContext.PrimaryKeyValuePairs.Add("UUID",uuid);
                    Resolvers.IQueryEngine queryEngine = _queryEngineFactory.GetQueryEngine(sqlMetadataProvider.GetDatabaseType());

                    using JsonDocument? frcResponse = queryEngine.ExecuteAsync(frContext).Result;

                    if ( null == frcResponse ) { 
                        throw new DataApiBuilderException("Permission denied",HttpStatusCode.Forbidden,DataApiBuilderException.SubStatusCodes.AuthorizationCheckFailed);
                    }

                    uuidEncountered = true;
                    return;
                }
