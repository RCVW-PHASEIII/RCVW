...
def hristatus_status(azservicebus: func.ServiceBusMessage):
    ...
    # Check for any IEEE status messages
    data = json.loads(azservicebus.get_body().decode('utf-8'))

    if len(data) == 1:
        # Status topic name. Just read the data
    ...
    if 'heartbeat' in data:
        if 'crossings' not in data:
            data['crossings'] = [ {
                ...
                'preemption': None,
                'egress-gates': None,
                'egress-gates-up': None,
                'egress-gates-down': None,
                'ingress-gates': None,
                'ingress-gates-up': None,
                'ingress-gates-down': None,
                'preemption-design-time': None
            } ]

        for xing in data['crossings']:
            if 'trains' not in xing:
                xing['trains'] = [ {
                    'sequence': None,
                    'direction': None,
                    'island-occupied': None,
                    'warning-design-time': None,
                    'estimated-warning-time': None
                } ]
            ...
            for train in xing['trains']:
                conn.cursor().execute("INSERT INTO RBS_IEEE1570_4909_MESSAGE " +
                                      ...
                                      "[xingEgressGates], [xingEgressGatesUp], [xingEgressGatesDown], " +
                                      "[xingIngressGates], [xingIngressGatesUp], [xingIngressGatesDown], " +
                                      "[xingPreemptionDesignSeconds], [trainSequence], [trainDirection], " +
                                      "[trainIslandOccupied], [trainWarningDesignTime], [trainEstimatedWarningSeconds]) " +
                                      "VALUES (CONVERT(uniqueidentifier, ?), ?, ?, ?, ?, ?, ?, ?, "
                                      "?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                                      str(uuid.UUID(hex=azservicebus.message_id)), azservicebus.label,
                                      int(data['heartbeat']), len(data['crossings']), len(xing['trains']),
                                      int(xing['sequence']), int(xing['operational']), int(xing['detection']),
                                      int(xing['flashers']), int(xing['preemption']), int(xing['egress-gates']),
                                      int(xing['egress-gates-up']), int(xing['egress-gates-down']),
                                      int(xing['ingress-gates']), int(xing['ingress-gates-up']),
                                      int(xing['ingress-gates-down']), int(xing['preemption-design-time']),
                                      int(train['sequence']), str(train['direction']),
                                      int(train['island-occupied']), int(train['warning-design-time']),
                                      int(train['estimated-warning-time']))
